...
# 결과 — 사전 예측이 부호에서 틀렸다 (2026-09-10 18:52, 반복 1, 6/6)

채점 `FS_CHAT_TBT_MS=25 FS_DR_TBT_MS=50 FS_SWE_TBT_MS=38 FS_CHAT_TTFT_S=2.5 FS_DR_TTFT_S=5 FS_SWE_TTFT_S=3.5`.

| req/s | **FluidServe** off / adm / 거절% / goodput | **PolyServe** | 격차 |
|---|---|---|---|
| 100 | **99.9** / 99.9 / 0.0 / 54,354 | 95.5 / 95.5 / 0.0 / 53,604 | **+4.4** |
| 160 | **99.0** / 99.8 / 0.8 / 85,893 | 74.3 / 75.6 / 1.7 / 65,435 | **+24.7** |
| 210 | **71.7** / 99.4 / 27.6 / **84,603** | 45.9 / 60.6 / 24.1 / 51,125 | **+25.8** |

goodput 격차 **+1.4% / +31.3% / +65.5%**.

## R.1 세 예산 설정 — **조일수록 우위가 커지고 우리 절대값은 안 움직인다**

| req/s | | 기존 (50/100/75, TTFT 5/10/7 s) | (A) 토큰당 절반 | **(B) 둘 다 절반** |
|---|---|---|---|---|
| **160** | FluidServe | 99.4 | 99.5 | 99.0 |
| | PolyServe | **99.6** | 76.2 | 74.3 |
| | **격차** | **−0.2** | **+23.3** | **+24.7** |
| **210** | FluidServe | 69.5 | 67.7 | 71.7 |
| | PolyServe | **92.6** | 62.3 | 45.9 |
| | **격차** | **−23.1** | **+5.4** | **+25.8** |

**우리 offered가 세 설정에서 99.4/99.5/99.0과 69.5/67.7/71.7로 사실상 평평하고, PolyServe만
99.6→76.2→74.3과 92.6→62.3→45.9로 내려간다.**

## R.2 ⚠ 사전 예측(§5)이 틀렸다 — 부호가 반대다

실행 전에 적은 예측은 **(B) − (A)가 −0.1~−0.4 / −0.5~−0.6 / −1.2~−8.6점, 거절 최대 +7.4점 상승**
이었다. 실측은 **+0.4 / −0.5 / +4.0**이고 **210에서 거절이 30.6% → 27.6%로 내려가고 admitted가
97.9 → 99.4로 올랐다.** 셋 중 **160의 −0.5만 예측 범위 안**이다.

**예측이 놓친 것**: 첫토큰 예산은 채점에만 들어가는 것이 아니라 **`canWait`의 마감**
(`ttftSloMs − after − recheckMs`, `fluidserve.go:2686`)에도 들어간다. 예산을 조이면 **마감이
짧아져 보류를 덜 하고 더 일찍 결정한다.** 그러면 오래 붙들렸다가 결국 거절되는 요청이 줄고
(거절 하락), 배치될 때 남은 예산이 많다(admitted 상승). **예측은 "시험이 어려워진다"만 셈했고
"정책의 행동이 바뀐다"를 안 셌다.**

⚠ **이것은 그럴듯한 설명이고 확인하지 않았다.** 확인 방법: 두 arm에서
`scheduler_fluidserve_decisions_total{decision="pend"}`와 배치 시점 `waited` 분포를 비교한다.
클러스터가 필요 없다. **확인 전에는 기전으로 인용하지 않는다.**

**이것은 오늘 밤 네 번째로 같은 모양이다** — 어떤 양을 조이면 **그 양이 들어가는 판정 자리를
통해 정책의 행동 자체가 바뀌고**, 채점만 생각한 예측이 부호에서 틀린다.

## R.3 인용 조건

1. **반복 1회.** 편차 최대 4.6점이므로 **160의 +24.7과 210의 +25.8은 안전하고 100의 +4.4는 경계선**이다.
2. **(B)에는 배수의 유도가 없다.** (A)의 토큰당 절반은 두 독립 측정(1.96 / 1.97)에서 나왔지만
   첫토큰 절반은 유도가 아니라 **타당성 검사**다. **논문에서는 (A)가 주 결과이고 (B)는
   "더 조여도 성립한다"는 보조로만 쓴다.**
3. **이 표는 기존 예산으로 채점한 표와 같은 자리에 못 놓는다.** 채점기가 경고 여섯을 찍는다.
4. **(B)의 FluidServe arm은 요청 스트림이 (A)와 바이트 동일하지 않다** — 첫토큰 예산이
   요청의 `priority` 정수에 포장돼 전달되는 유일한 경로라서다. 프롬프트·길이·믹스·시드는 같다.
5. ⚠ **옛 분석 스크립트 다섯 개(`exp14_per_class_slo.py`, `exp09_goodput_tokens.py`,
   `plot_exp09_swe.py`, `plot_exp07_theta.py`, `slo_sliding_window.py`)가 TTFT 5초를
   하드코딩**하고 있다. **이 run들에 그것들을 쓰면 조용히 옛 예산으로 채점한다.**
   표준 세트(`exp22`/`exp27`/`exp38`)는 깨끗하다.
