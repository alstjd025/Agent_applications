...
**⚠ 한 시간 trace는 반쪽만 있다.** `fsnoaff/full`이 시작 직후 0행으로 죽었다 —
노드가 **DiskPressure**에 걸려 게이트웨이 파드가 축출되는 중이었다(§7 참조).
`fluidserve/full`은 정상(363,466행, 61.0분). **따라서 아래 판정은 전부 정적 12조건에서
나온 것이고, 한 시간 trace의 arm 간 비교는 없다.**
...
- **한 시간 trace의 arm 간 비교가 없다**(`fsnoaff/full` 실패). 정적에서 본 것이 움직이는
  워크로드에서도 성립하는지는 미측정이고, **다시 돌려야 한다.**
- **§58.3의 인과 주장**("분리가 chat 페이스를 예산 안에 두는 기전이다")은 이제 정적
  조건에서 **뒷받침된다** — 분리를 끄면 chat 토큰당 시간이 39.4 → 44.1 ms로 오른다.
  다만 그것이 **점수 이득의 얼마를 설명하는지**는 §6.4의 이유로 못 가른다.
