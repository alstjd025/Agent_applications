#!/bin/bash
# EXP-137 SNAPSHOT of run_exp134_capalone.sh, taken 2026-09-14. ONE arm added.
#
# THE QUESTION. Class preference and the per-class instance cap are the only two
# mechanisms in this policy that read the request's class (class-pin and
# class-harm are both off in every arm), and removing them ONE AT A TIME moves
# the score in OPPOSITE directions: the preference is worth +1.9 to +5.6 points
# at this cell and the cap COSTS 10.3 to 12.5. Their sum is therefore not
# something the existing table can be read off, and EXP-133 already showed that
# leave-one-out understates an element that another element is covering for.
# What "class-aware placement, in total" is worth has not been measured.
#
#   noaffnocapt75   the deployed configuration with --fluidserve-enable-affinity
#                   false AND --fluidserve-class-instance-cap false, and nothing
#                   else changed. FS_CAP_WINDOW_MULT stays at 3.0 and is inert
#                   with the cap off; it is pinned so the arm differs from the
#                   control in exactly two flags as the startup line reports them.
#
# WHY THE TWO ARE NOT THE SAME MECHANISM, despite both reading the class. The
# preference is a WEIGHT in sortCandidates -- it reorders feasible destinations
# and removes none. The cap is a FILTER -- past a demand-derived limit, and only
# while another class with arrivals is short of its own, it deletes candidates
# from the list before sorting, exactly as applyClassPin does. One cannot cost
# capacity and the other can, which is why their measured signs differ.
#
# ⚠ RUNS ON LLAMA. Every number this is compared against -- deployed 73.3-75.5,
# preference off 69.9-71.4, cap off 85.6-85.8, all three off 82.9-84.4 -- is
# Llama-3.1-70B at 2,100 rpm. The chain switches the fleet back from Qwen before
# it starts and refuses to run if the four ports are not serving Llama.
# FS_PROFILE_DIR is deliberately NOT set here: the default is the Llama tables.
# EXP-134 SNAPSHOT of run_exp133_cumulative.sh, taken 2026-09-14. ONE arm added.
#
# EXP-133's staircase says that removing the per-class instance cap gains 5.6 to
# 9.7 points -- but it says that CUMULATIVELY, with the class preference and
# deferral already removed. EXP-107 adopted the cap with the preference ON, so
# the number that decides whether the deployed configuration should keep it is
# the cap removed BY ITSELF, and that cell does not exist. Deferral's own
# leave-one-out was measured in EXP-131 (fsv3t75nopend, 74.1/75.6 against a
# control of 73.6/75.5, i.e. nothing), so the cap is the only hole.
#
#   nocapt75   the deployed configuration with --fluidserve-class-instance-cap
#              false and nothing else changed. FS_CAP_WINDOW_MULT stays at 3.0
#              and is inert with the cap off; it is pinned rather than unset so
#              that this arm differs from the control in exactly one flag as the
#              startup line reports it.
#
# ⚠ What this arm CANNOT settle. EXP-107 adopted the cap for a redistribution it
# measured on the hour trace -- deep research gaining 8 to 12 points in the
# chat-heavy segments -- and a static cell at one arrival rate does not measure
# that. A loss here is a reason to re-examine the adoption on the hour trace, not
# a reason to drop the cap.
# EXP-133 SNAPSHOT of run_exp131_ablation.sh, taken 2026-09-14. Six arms added,
# cum0 through cum5, each removing one more of the mechanisms that FluidServe has
# and llm-d does not. They are CUMULATIVE, not leave-one-out, and that is the
# whole point of the experiment.
#
# WHY LEAVE-ONE-OUT WAS THE WRONG SHAPE. EXP-131 removed six elements one at a
# time and three of them cost nothing measurable: the remaining-budget condition
# 0 points, deferral 0 points, the class preference 1 to 2 on the hour trace.
# EXP-132 then measured WHY the class preference reads so small: the share of
# (instance, time) pairs holding no chat request is 2.1 instances of four with
# the preference on and 1.8 with it off, so the feasibility test produces 86% of
# that separation by itself. When two mechanisms independently achieve the same
# protection, removing either one alone costs nothing and removing both costs a
# great deal -- and leave-one-out cannot see that. This design is deliberately
# redundant, so leave-one-out understates every element in it.
#
# The accounting that motivates this: on the hour trace FluidServe scores 78.7
# against llm-d's 47.7 and 54.0, a gap of 25 to 31 points, while everything
# EXP-131 and EXP-132 attributed to a FluidServe-specific mechanism sums to under
# 10. Something holding twenty points has not been isolated.
#
# THE ORDER. Strip only what is OURS, most peripheral first, ending at a decision
# whose shape is llm-d's. What is shared is not stripped: taking the minimum
# budget among the requests already on an instance is in llm-d too
# (running_request_tpot_slo_queue.go), as is admission by refusal and prefix
# affinity, so none of those is a differentiator and none is removed here.
#
#   cum0  the deployed configuration, verbatim. An IN-SESSION control: it must
#         land inside EXP-108's 73.6-75.5, and if it does not, nothing below it
#         can be read, because the two sessions would then differ in something
#         other than the arms. This is not redundant with EXP-108 -- EXP-132
#         found preemption differing 2.1 to 2.6 times between these sessions with
#         no cause identified, and this arm is what says whether the SCORE also
#         moved.
#   cum1  - class preference. llm-d has prefix affinity, not class affinity.
#   cum2  - deferral. llm-d places or refuses at arrival, it never holds.
#   cum3  - the per-class instance cap, the guardrail EXP-107 adopted.
#   cum4  - the KV flux projection, so an instance is judged on its current
#         occupancy alone. NOTE this leaves costOf untouched: enableFlux only
#         zeroes inflow and outflow (fluidserve.go:1316), so the arriving
#         request is still charged for the KV it will occupy.
#   cum5  - that charge as well, by setting the horizon to 1 step. costOf is
#         min(horizonSteps, expectedToks), so a horizon of 1 charges the prompt
#         plus one token, which is what llm-d's predictor is given
#         (generatedTokenCounts[i] = 1, prediction.go:71). With the projection
#         already off at cum4, the horizon no longer means anything else, so
#         this step isolates the future-KV charge cleanly.
#
# HOW TO READ IT. The staircase says where the drop is. If cum5 lands near
# llm-d's 47.7-54.0 the gap is accounted for by these mechanisms; if it does not,
# what remains is not a mechanism difference but a model difference -- ours is an
# analytic step-time model fitted offline, theirs is a predictor trained online --
# and that has to be said rather than left implied.
#
# ⚠ Every arm pins FS_INCUMBENT_BALANCE=true and the flags the control pins, so a
# cumulative step differs from the step above it in exactly one variable. Read
# the case block as a diff.
#
...
set_arm() {  # $1 = fluidserve | polyserve | polyservep* | slo | loadbalance
  ...
  case "$1" in
    ...
    # EXP-133 cumulative ablation. cum0 repeats the control's exports verbatim;
    # each later arm adds ONE more removal to the one above it.
    noaffnocapt75) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_INSTANCE_CAP=false FS_AFFINITY=false ;;
    nocapt75) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_INSTANCE_CAP=false ;;
    cum0)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true ;;
    cum1)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_AFFINITY=false ;;
    cum2)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_AFFINITY=false FS_PEND=false ;;
    cum3)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_AFFINITY=false FS_PEND=false FS_INSTANCE_CAP=false ;;
    cum4)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_AFFINITY=false FS_PEND=false FS_INSTANCE_CAP=false FS_FLUX=false ;;
    cum5)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_AFFINITY=false FS_PEND=false FS_INSTANCE_CAP=false FS_FLUX=false FS_HORIZON=1 ;;
    ...
    # EXP-131. Leave-one-out from the adopted configuration above. Every arm
    # below is fsv3capgnofrct75 with exactly ONE mechanism removed, so the
    # control for all four is the fsv3capgnofrct75 cell already on disk from
    # EXP-108 at the same rate. Each arm pins FS_INCUMBENT_BALANCE in whichever
    # direction it wants so the start-up line carries a positive confirmation
    # either way, which is the same discipline FS_FORCE_MARGIN follows.
    #
    # WARNING --fluidserve-incumbent-balance exists only in the binary built for
    # this experiment (md5 008e0a0c). pflag exits on an unknown flag, so pinning
    # it in a driver that runs an older binary puts the scheduler into
    # CrashLoopBackOff and every later condition then fails to read a start-up
    # line (EXP-48). Do not copy these arms into another driver.

    # The second pace condition: the pace after admitting must also fit the
    # tightest REMAINING budget on the instance, not only the tightest nominal
    # one. Removing it leaves the nominal gate, which is the form the baselines
    # use, so this arm prices the rate-versus-stock difference directly.
    fsv3t75noinc)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=false ;;
    # Class preference. Turns off the sort key; the harm class term is already
    # off in every arm here via FS_CLASS_HARM=false, so this removes one thing.
    fsv3t75noaff)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_AFFINITY=false ;;
    # Hold-and-retry. A request that cannot be placed now is decided at once
    # instead of being held and asked again on the gateway's retry interval.
    fsv3t75nopend) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_PEND=false ;;
    # Admission control. FS_SHED=false with FS_FORCE=false panics at start-up on
    # purpose -- a request no instance can take would then have no exit and
    # would sit until the gateway ceiling -- so removing rejection means putting
    # the forced placement back. That makes this arm a two-flag change, and the
    # caption has to say so. It also does NOT reach zero refusals: the gateway
    # still drops a request at its 35,000 ms ceiling.
    fsv3t75norej)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_SWE_TBT_MS=75 FS_INCUMBENT_BALANCE=true FS_SHED=false FS_FORCE=true ;;

    # EXP-131b. Loosen ONLY the instance-coupling term of the pace gate, and
    # leave everything else at the adopted configuration.
    #
    #   gate = min(own nominal, instance minimum x gateSlack) x 0.90
    #
    # gateSlack multiplies the instance minimum alone, so 1.1111 makes that
    # branch effectively x1.00 (50 x 1.111 x 0.90 = 49.995) while the request's
    # own budget is still checked
    # at x0.90. NOTE 1.1111 cannot be used: the verifier compares the value it
    # asked for against the start-up line as a STRING, and that line formats the
    # float with %.3f, so 1.1111 is reported as 1.111 and every condition aborts.
    # That is the smallest change that can make the remaining-budget
    # condition reachable: measured on the runs already on disk, the tightest
    # REMAINING budget is below the instance gate on 0.4% of instance-samples at
    # x0.90 and 16.9% at x1.00 (35 req/s, 2 repeats). The cliff is just above:
    # x1.10 would be 85.7%, so if x1.00 does not activate the term this arm's
    # own counters say so and the next step is a larger multiplier.
    #
    # The instance minimum is NOT chat-specific -- it is chat 50 on 49.0% of
    # samples and swe 75 on 43.9% -- so this loosens whichever class sets it.
    # What changes is placements by requests whose own budget is looser than
    # that minimum; a chat request onto a chat-holding instance is unaffected
    # because its own 50 still wins the min().
    #
    # Read the RESULT from two places: the score against the control, and
    # scheduler_fluidserve_infeasible_sole_total{reason="incumbents"}, which
    # says whether the term became the binding one at all.
    fsv3t75gs111)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_GATE_SLACK=1.111 ;;
...
declare -A ARMMIX=(
  ...
  # EXP-133 cumulative ablation -- all six take the same workload file as the
  # control, because they ARE the control with things removed.
  [cum0]=t75
  [nocapt75]=t75
  [noaffnocapt75]=t75
  [cum1]=t75
...
case "${1:-}" in
  ...
  onerep)
    ...
    SESSION_PREFIX="${SESSION_PREFIX_BASE:-exp108}r${rep}" \
