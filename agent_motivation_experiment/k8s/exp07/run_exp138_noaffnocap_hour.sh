#!/bin/bash
# EXP-138 SNAPSHOT of run_exp135_nocap_hour.sh, taken 2026-09-14. ONE arm added.
#
# EXP-137 measured, at a static 35 req/s, that turning the class preference off
# costs 4.0 points while the per-class instance cap is ON and costs NOTHING while
# it is off: 73.3-75.5 -> 69.9-71.4 with the cap, 85.6-85.8 -> 85.6-85.7 without
# it. The preference's value at that cell is therefore something the cap creates
# -- the cap deletes candidates and the preference reorders what is left, so with
# nothing deleted the reordering changes no total.
#
# Whether that holds on the hour trace is a separate question, because the cap's
# TOTAL effect there is zero (EXP-135: 78.4-78.5 against a control of 78.5/77.8)
# while its per-class effect is not (deep research 65.7-66.0 against 68.0-69.1).
# With the cap costing nothing in total, the argument that the preference's value
# depends on it has no obvious hold.
#
#   noaffnocaphour   the deployed configuration with --fluidserve-enable-affinity
#                    false AND --fluidserve-class-instance-cap false, on the hour
#                    mix-shift trace. Exports repeat nocaphour's verbatim and add
#                    FS_AFFINITY=false, so a diff against that arm is the whole
#                    description.
#
# The other three corners are already on disk and are not re-run: control is
# EXP-109 (2 repeats), preference off is EXP-132 (1 repeat), cap off is EXP-135
# (2 repeats).
#
# ⚠ READ PER CLASS, NOT ONLY THE TOTAL. On the hour trace the cap's whole effect
# is per class. If the total does not move here either, the question is what deep
# research does -- statically, removing both dropped it from 64-66 to 59.8-60.2,
# further than removing the cap alone.
# EXP-135 SNAPSHOT of run_exp132_variations.sh, taken 2026-09-14. ONE arm added,
# and it re-opens an adoption decision.
#
# EXP-134 measured the per-class instance cap removed by itself at static
# 35 req/s: attainment 85.6 and 85.8 against the deployed configuration's 73.3
# and 75.3, rejection 11.9-12.1% against 22.0-23.4%, goodput 14,633-14,668
# against 12,990-13,259. Two repeats each and the treatment pair is 0.2 apart.
# The cap is costing 10 to 12 points there.
#
# That does not settle it, because EXP-107 did not adopt the cap for a static
# cell. It adopted it for a redistribution measured on THIS trace: in the
# chat-heavy segments deep research gained 8 to 12 points. A static cell at one
# arrival rate cannot see that, and the hour trace is where the decision was
# made, so it is where it has to be re-examined.
#
#   nocaphour   the deployed configuration with --fluidserve-class-instance-cap
#               false and nothing else changed, on the hour mix-shift trace.
#               FS_CAP_WINDOW_MULT stays at 3.0 and is inert with the cap off;
#               it is pinned so the arm differs from the control in exactly one
#               flag as the startup line reports it.
#
# The control is EXP-109's two repeats of fsv3capgnofrct75, already on disk
# (all arrivals 78.5 / 77.8 under t75 scoring). Not re-run.
#
# WHAT TO LOOK AT, and it is not only the total. The question EXP-107 answered
# was per class and per segment: if the total improves but deep research loses
# its 8-to-12-point gain in the chat-heavy segments, the cap is buying isolation
# at a price, and that is a design choice rather than a defect. Read
# exp133_staircase.py's per-class rows AND the segment decomposition in
# EXP-132 section 6.5 before concluding anything.
# EXP-132 SNAPSHOT of run_exp109_hour.sh, taken 2026-09-13. Three arms added,
# each one FluidServe v0.3 + class-instance cap + explicit rejection (that is,
# exactly the `fsv3capgnofrct75` configuration EXP-109 measured) with ONE thing
# changed. The control is EXP-109's two repeats already on disk; it is not re-run.
#
#   orct75      The per-request output-length ORACLE replaces the class length
#               distribution. Two scheduler flags, because the length is read in
#               five places and leaving any of them on the class curve would make
#               the arm uninterpretable:
#                 --fluidserve-oracle-length  the arriving request's charge, its
#                     remaining decode in the deadline test, and the registry's
#                     remaining-token count.
#                 --fluidserve-oracle-flux    the KV flux projection. inflow
#                     becomes sum over residents of min(remaining, horizon)
#                     instead of an unconditional horizon per resident, and
#                     outflow's completion probability becomes 0 or 1 instead of
#                     the class survival curve (so the z-safety variance term
#                     also goes to zero -- complete information needs no margin).
#               ⚠ The table covers 79.7% of the arrivals of the other repeat of
#               the same condition (chat 84.7 / deepresearch 67.7 / swe 74.5) and
#               86.0% of the requests that produced output. A request with no
#               entry falls back to the class distribution in BOTH flags, so this
#               arm is "an oracle for four requests in five", not a full oracle.
#               On the requests it does answer it is exact for 94.0% of them, with
#               a median absolute error of 0 tokens.
#
#   noafft75    --fluidserve-enable-affinity=false. The class preference is worth
#               +3.3~+5.6 points of all-arrivals attainment at a static 35 req/s
#               and +9.1~+11.4 at 45 (EXP-131, 3 and 2 repeats). This asks whether
#               it is still worth that when the class mix moves every 15 minutes.
#               ⚠ It also removes the class term of `harm`, but every arm here
#               pins FS_CLASS_HARM=false, so in practice only the candidate
#               ordering changes.
#
#   flatlent75  No per-class length knowledge at all: a profile directory in which
#               all three classes share ONE deterministic output length of 532
#               tokens -- the mix-weighted mean of the 79,100 completed requests
#               of 260831_2015_exp109r1_fsv3capgnofrct75_shift. survival is the
#               step [1,1,0,0] on the grid [0,531,532,100000], so completionProb
#               is 0 or 1 and expectedRemaining is 532-j for every class.
#               The tier keys (25/50/100) are kept, because fluidserve_profile.go
#               indexes the classes by them, and ttft.json / tpot.json are copied
#               unchanged, because those are properties of the engine and the
#               hardware rather than of the workload. Selected with FS_PROFILE_DIR,
#               so no code path differs between this arm and its control.
#
# ⚠ ONE REPEAT EACH. Every number these produce is a single measurement and the
# run-to-run movement of an hour trace on this workload is up to 4.6 points, so a
# difference smaller than that says nothing.
#
# ⚠ The runner template is a snapshot too (runner-dyn-exp132.template.yaml),
# because LENGTH_ORACLE_FILE had to be added to it and every other driver that
# uses the shared runner-dyn.template.yaml would then pass the literal
# "__ORACLE__" to agent.py, which exits on an unreadable path.
#
...
set_arm() {
  ...
  unset FS_PEND FS_SHED FS_AFFINITY FS_AFFINITY_WEIGHT FS_AFFINITY_METRIC FS_CLASS_PIN \
        ...
        FS_ORACLE_LEN FS_ORACLE_FLUX FS_PROFILE_DIR LENGTH_ORACLE_FILE
  ...
  case "$1" in
    ...
    # EXP-132. Three variations of fsv3capgnofrct75 below. Each repeats that
    # arm's exports verbatim and then changes exactly one thing, so a diff
    # against the control line is the whole description of the arm.
    noaffnocaphour) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INSTANCE_CAP=false FS_AFFINITY=false ;;
    nocaphour)   policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_INSTANCE_CAP=false ;;
    orct75)      policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false \
                                          FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true \
                                          FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true \
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
                                          FS_SWE_TBT_MS=75 \
                                          FS_ORACLE_LEN=true FS_ORACLE_FLUX=true \
                                          LENGTH_ORACLE_FILE=/work/traces/length_oracle_shift62_hour.json ;;
    noafft75)    policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false \
                                          FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true \
                                          FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true \
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
                                          FS_SWE_TBT_MS=75 \
                                          FS_AFFINITY=false ;;
    flatlent75)  policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false \
                                          FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true \
                                          FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true \
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
                                          FS_SWE_TBT_MS=75 \
                                          FS_PROFILE_DIR=llama31-70b-b200-tp2-flatlen ;;
...
run_cell() {  # $1 arm, $2 variant (ablation|full)
  ...
  case "$arm" in
    fsv3capgnofrct75|orct75|noafft75|flatlent75|nocaphour|noaffnocaphour)
      wcfg=/work/workload_configs/mix_dyn60_shift_m2Am1B_b1045_t75.json ;;
    polyservept75|slot75|vllmcachet75|llmdslot75)
      wcfg=/work/workload_configs/mix_dyn60_shift_m2Am1B_b1045_t75fair.json ;;
  esac
  ...
  kubectl -n llumnix delete job "$job" --ignore-not-found >/dev/null
  # EXP-132. LENGTH_ORACLE_FILE is empty for every arm but orct75; agent.py
  # treats the empty string as "no table" and sends no hint. It is checked for
  # existence here rather than in the pod, because a missing table would
  # otherwise turn into a runner crash after the fleet has already restarted.
  if [ -n "${LENGTH_ORACLE_FILE:-}" ]; then
    [ -s "${HOSTWORK}${LENGTH_ORACLE_FILE#/work}" ] \
      || { echo "[exp30] missing oracle table $LENGTH_ORACLE_FILE"; return 1; }
  fi
  sed -e "s/__JOBNAME__/$job/" -e "s/__SESSION__/$session/" -e "s/__ARM__/$arm/" \
      -e "s#__TRACE__#$trace#" -e "s#__WCFG__#$wcfg#" \
      -e "s#__ORACLE__#${LENGTH_ORACLE_FILE:-}#" \
      runner-dyn-exp132.template.yaml | kubectl apply -f - >/dev/null
