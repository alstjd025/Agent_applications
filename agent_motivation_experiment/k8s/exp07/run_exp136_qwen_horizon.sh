#!/bin/bash
# EXP-136 SNAPSHOT of run_exp131j_h1.sh, taken 2026-09-14. No new arms: the six
# horizon arms and the projection-off arm already exist here. What changes is the
# MODEL and the arrival rate, and one line pins the profile directory.
#
# THE QUESTION. On Llama-3.1-70B the horizon curve peaks at 25-50 and the
# deployed value of 100 sits 7 to 10 points below the peak, and turning the KV
# projection off at a horizon of 50 is 10 to 12 points above the deployed
# configuration. Both are single-model results. If they reproduce on a second
# model of the same size and fleet shape, "the deployed horizon is not the
# optimum" does not rest on Llama; if the optimum moves, the horizon has to be
# chosen per model, and that is worth knowing too.
#
# THE ARRIVAL RATE. 1,344 rpm, which is 2,100 x 0.64 -- the same factor EXP-113
# used and validated when it scaled this workload's hour trace for Qwen, chosen
# so that the average load sits at the same multiple of the knee. Llama's knee is
# 28.4 req/s and Qwen's is 18.3 (EXP-113 section 1), so 35 req/s is 1.23x on
# Llama and 22.4 req/s is 1.22x on Qwen. Reusing EXP-113's factor rather than
# deriving a new one keeps the two model comparisons on one scaling.
#
# THE BUDGETS DO NOT MOVE. swe keeps TTFT 7 s + 75 ms/token and the other two
# classes keep theirs. Adjusting arrival rate to capacity is standard; moving the
# deadline with the model is not, and it reads as fitting the deadline to the
# system. EXP-113 held them fixed for the same reason.
#
# ⚠ FS_PROFILE_DIR IS PINNED BELOW AND THAT IS NOT OPTIONAL.
# set_scheduler_profiling.py defaults to the Llama tables. Running Qwen engines
# against Llama's latency profile is a failure in which the scheduler starts
# normally, the settings verifier passes, and every predicted step time, every
# capacity ceiling and every length distribution is wrong. Nothing in the run
# would say so. The driver prints "profile tables from ..." per condition and
# that line is the check.
...
set_arm() {  # $1 = fluidserve | polyserve | polyservep* | slo | loadbalance
  ...
  # EXP-136. Every arm in this snapshot runs on Qwen2.5-72B, so the profile
  # directory is pinned once here rather than per arm. See the header: the
  # default is Llama and the mistake is silent.
  export FS_PROFILE_DIR=qwen25-72b-b200-tp2
  case "$1" in
    ...
    # EXP-131d. Turn off the KV flux projection: proj = kvLogical + inflow -
    # outflow becomes proj = kvLogical, so the memory predicate judges the
    # occupancy the instance has NOW instead of the one it is heading for.
    # This is the accounting itself, and it has never been run as an arm --
    # EXP-34 counted flips instead, on the grounds that the feedback would
    # swallow an A/B. The counters say it is worth trying: the projected and
    # the snapshot verdict disagree on 3.1-4.7% of candidate evaluations at the
    # adopted gate and 11.4% at the loosened one, against 0.4-2.7% for the
    # remaining-budget condition.
    fsv3t75noflux) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_FLUX=false ;;

    # EXP-131g. The projection with the missing arrivals term added back:
    # proj = kvLogical + inflow + arrivals - outflow, where arrivals is the
    # prompt tokens this instance received per horizon over the last N. It is
    # the fix for what EXP-131d measured -- turning the projection OFF is worth
    # +5.5 to +7.7 points here, which says the projection as shipped is a net
    # loss. EXP-124/125 measured N=1 on the eight-instance 8B fleet (admitted
    # 90.0 -> 96.9 at 210 req/s) and it has never been run on this fleet.
    # The question this arm answers is narrow: does FIXING the projection beat
    # TURNING IT OFF. If it does not clear +5.5, the projection should go.
    fsv3t75ah1)    policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_ARRIVAL_HORIZONS=1 ;;

    # EXP-131h. The left end of the horizon curve. 50/100/250/500 are monotone
    # (all arrivals 83.8 / 74.6 / 62.3 / 49.4, repeat 1), so the deployed 100 is
    # not the optimum and the question is where the turn is. Turning the
    # projection OFF entirely (FS_FLUX=false) scores 81.0-81.3, BELOW h=50, so
    # zero is not the answer either -- the optimum is between.
    fsv3t75h25)    policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_HORIZON=25 ;;

    # EXP-131i. Separates the two things the horizon does. costOf charges an
    # arriving request `min(horizon, expectedToks)` of future KV REGARDLESS of
    # enable-flux, so the fsv3t75noflux arm (horizon 100, flux off) differs from
    # fsv3t75h50 (horizon 50, flux on) in BOTH the charge and the projection and
    # cannot separate them. This arm holds the charge window at 50 and removes
    # only inflow/outflow.
    #   no difference  -> the projection terms barely matter and what works is
    #                     the length of the CHARGING window
    #   flux-on better -> the projection has value and 100 was just too long
    # Expected to be small: at h=50 the measured inflow - outflow is about
    # 8,300 - 13,400 = -5,100 tokens, roughly 1% of the pool.
    fsv3t75h50noflux) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_HORIZON=50 FS_FLUX=false ;;

    # EXP-131j. The smallest runnable horizon. 0 panics by design because the
    # horizon is a denominator in the capKv inversion, and h=1 is the nearest
    # thing to "judge on the present only": an arrival is charged its prompt
    # plus ONE token of growth, and inflow and outflow are one iteration each.
    #
    # It is NOT simply "less lookahead", because the horizon enters meanStepMs
    # with the opposite sense: f = (queued prefill chunks) / h saturates at 1,
    # so at h=1 a single queued chunk makes every iteration look like a
    # prefill-carrying one and the pace test becomes maximally pessimistic.
    #
    # This is the point that decides whether the curve has an interior optimum
    # (lookahead has a right amount, and 100 was too much) or is monotone in
    # "shorter is better" (lookahead does not help at all). 25 and 50 sit inside
    # each other's repeat spread, so the existing points cannot tell them apart.
    fsv3t75h1)     policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_HORIZON=1 ;;
    fsv3t75h10)    policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_HORIZON=10 ;;

    # EXP-131e. The planning horizon, swept. Deployed value is 100 engine
    # iterations; the control at this rate is the fsv3capgnofrct75 cell on disk.
    # Never swept before: FS_HORIZON exists and no arm has ever set it.
    #
    # 0 is NOT in the sweep -- the scheduler panics on it by design
    # (fluidserve.go:3199) because the horizon is a DENOMINATOR in the capKv
    # inversion (fluidserve_capacity.go:447). The "current occupancy only"
    # condition is FS_FLUX=false, which is a separate arm above: it zeroes
    # inflow and outflow while leaving the pace and capacity model at 100.
    #
    # The horizon enters four places at once, so this is not a one-term change:
    # inflow = nDecode x horizon, outflow = expectedOutflow(live, horizon),
    # the prefill-carrying share of the horizon in meanStepMs, and capKv.
    # A longer horizon therefore projects more growth AND more departures.
    fsv3t75h50)   policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_HORIZON=50 ;;
    fsv3t75h250)   policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_HORIZON=250 ;;
    fsv3t75h500)   policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false FS_SWE_TBT_MS=75 FS_HORIZON=500 ;;
