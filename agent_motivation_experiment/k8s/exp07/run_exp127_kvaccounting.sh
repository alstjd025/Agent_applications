#!/bin/bash
# EXP-123 SNAPSHOT of run_exp121_halfbudget.sh, taken 2026-09-10. It adds the two
# arms that halve the FIRST-TOKEN budgets on top of the per-token budgets EXP-121
# already halved, and the two workload files those arms need. No policy code
# changes, no flag changes, and no other arm's environment changes.
#
#   chat          first token 5000 -> 2500 ms, per token 50 -> 25 ms
#   deepresearch  first token 10000 -> 5000 ms, per token 100 -> 50 ms
#   swe           first token 7000 -> 3500 ms, per token 75 -> 38 ms
#
# WHY THIS EXISTS. EXP-121 halved the per-token budgets alone, on a derivation
# that is about per-token time alone, and deliberately left the first-token
# budgets where they were so that the two effects would not be confounded. This
# is the other half of that pair: it moves both, so that the difference between
# the two columns is the first-token budget and nothing else.
#
# ⚠ THERE IS NO MEASURED DERIVATION FOR HALVING THE FIRST-TOKEN BUDGET, and the
# experiment file says so in its first section. The eight-instance Llama-3.1-8B
# engine prefills 4.4x faster than the four-instance 70B one at idle (69.81
# against 15.76 tokens per millisecond), but under load the first-token time is
# dominated by queueing rather than by prefill compute, and queueing is produced
# by the policy rather than by the hardware: on the hour trace swe's first-token
# time is p50 6,589 ms under FluidServe and p50 371 ms under PolyServe, on the
# same fleet under the same 7 s budget. There is therefore no hardware ratio to
# normalise by, and this arm is a PLAUSIBILITY CHECK on a tighter overall service
# objective -- what Scorpio and AdaGen do when they relax deadlines for larger
# models without deriving the factor -- rather than the measured normalisation
# that the per-token half of the pair is.
#
# WHY IT IS WORTH RUNNING ANYWAY. The rule that actually fails on this fleet is
# swe's first-token budget. On the hour trace 47.67% of the swe requests
# FluidServe accepted miss the 7 s first-token budget and 0.09% miss the 75 ms
# per-token one, so halving the per-token budget alone leaves the dominant
# failure mode untouched and halving the first-token budget makes it strictly
# harder. This arm is predicted to score WORSE than EXP-121's, and the experiment
# file states by how much and why before the run.
#
# WHERE THE FIRST-TOKEN BUDGET LIVES, WHICH IS NOT WHERE THE PER-TOKEN ONE DOES.
# No --fluidserve-* flag carries a first-token budget at all. The client packs it
# into the OpenAI `priority` field as ttft_ms*1000 + tbt_ms
# (workloads/swe_bench_coding/agent.py _priority, priority_mode=deadline in the
# runner template), the gateway unpacks it in types.DecodePackedSlo, and
# fluidserve.go:566 and polyserve.go read the per-request value. So BOTH arms are
# told the first-token budget through their workload file, and BOTH arms need a
# new one -- unlike EXP-121, where FluidServe kept mix_short_m1_t75.json because
# its per-token budgets arrive through --fluidserve-class-budgets.
#
#   FluidServe  mix_short_m1_ftc2500d5000s3500.json: ttft_ms 2500/5000/3500 with
#               tbt_ms left at the TIER KEYS 50 / 100 / 25, which is what
#               fluidserve_profile.go:173 indexes the length model by. Per-token
#               budgets still arrive as --fluidserve-class-budgets
#               25:decode:38,50:decode:25,100:decode:50, identical to EXP-121.
#   PolyServe   mix_short_m1_c25d50s38ftc2500d5000s3500fair.json, which states
#               both budget sets literally, because PolyServe reads
#               slo.<class>.tbt_ms as the per-token target and
#               set_scheduler_profiling.tier_by_class() derives
#               --polyserve-tier-decode-tokens from that same field.
#   scoring     FS_CHAT_TBT_MS=25 FS_DR_TBT_MS=50 FS_SWE_TBT_MS=38
#               FS_CHAT_TTFT_S=2.5 FS_DR_TTFT_S=5 FS_SWE_TTFT_S=3.5, for BOTH
#               arms and for every table built from these runs.
#               exp22_fluidserve.py prints a banner for each on import. Numbers
#               scored this way must never sit in a table with numbers scored at
#               first-token 5 / 10 / 7 s or at per-token 50 / 100 / 75 ms.
#
# ONE MECHANICAL CHANGE beyond the arms: the runner Job's name prefix is
# "bench-runner-" rather than "bench-runner-exp106-". These arm names are 36 and
# 39 characters, the Job controller names each pod "<job>-<5 characters>", and a
# pod name is a single DNS label capped at 63, so the longer prefix would leave
# the FluidServe arm's pod name at 65 and the Job would create no pods at all.
# The string "bench-runner" is kept because every chain's idle gate counts
# incomplete Jobs whose name contains it.
#
# (Original header follows.)
# EXP-121 SNAPSHOT of run_exp120_membrake.sh, taken 2026-09-10. It adds two arms
# that run the ADOPTED FluidServe v0.4 candidate and the paper-fidelity PolyServe
# with every PER-TOKEN budget halved and every FIRST-TOKEN budget unchanged, and
# it adds the workload file PolyServe needs to be told those budgets. No policy
# code changes and no other arm's environment changes.
#
# WHY. On this eight-instance Llama-3.1-8B TP=1 fleet the per-token budgets are so
# loose relative to the engine that the KV pool fills before any per-token budget
# is violated, so FluidServe's per-class pace model has nothing to bind on. The
# crossover footprint
#
#     T* = C(c_n + rho/R) / ((B - c0)s - C c_kv)
#
# is the resident KV footprint per request at which the pace ceiling and the
# memory ceiling coincide, and memory binds when the workload's footprint exceeds
# it. At the chat budget B = 50 ms this fleet has T* = 2,323 tokens against a
# measured footprint of 4,001, so memory binds. B appears only in the denominator,
# which falls from about 19.6 to about 4.6 when B halves, so T* rises to roughly
# 9,900 and the fleet becomes pace-bound. Halving the per-token budgets is
# therefore predicted to flip WHICH CEILING BINDS, which is the axis under test.
#
# WHY A FACTOR OF TWO, AND WHY ONLY THE PER-TOKEN BUDGETS. Two independent
# measured normalisations between the four-instance Llama-3.1-70B TP=2 fleet and
# this one agree on about 2: the ratio of budget to per-token time at a full KV
# pool is 1.96, and the ratio of budget to loaded p50 per-token time under a
# memory-comfortable policy is 1.97. The derivation above is about per-token time
# only. A first-token budget would need its own derivation, and moving both at
# once would confound the axis, so ttft_ms stays 5000 / 10000 / 7000.
#
#   chat           50 -> 25 ms per token
#   deepresearch  100 -> 50 ms per token
#   swe            75 -> 38 ms per token
#
# WHY 38 AND NOT 37.5. parseClassBudgets in
# pkg/scheduler/policy/fluidserve_registry.go reads the budget field with
# strconv.Atoi and fluidserve.go panics during start-up on a parse error, so the
# policy cannot be given a fractional budget at all. 38 is the nearest whole
# millisecond to 75/2, and every other place that states swe's budget uses the
# identical 38: the PolyServe workload file, the arm environment, and the scoring
# environment. Scoring at 37.5 while the policy runs 38 is the two-budget failure
# CLAUDE.md group A and EXP-105 both record.
#
# THE THREE PLACES THIS BUDGET LIVES, AND WHY THEY DIFFER BY ARM.
#   FluidServe  keeps mix_short_m1_t75.json UNCHANGED and receives all three
#               budgets through --fluidserve-class-budgets
#               25:decode:38,50:decode:25,100:decode:50. Its slo.<class>.tbt_ms
#               values (25 swe / 50 chat / 100 deepresearch) are TIER KEYS, not
#               budgets: the length model in fluidserve_profile.go indexes classes
#               by classes[].tpot_slo_ms with exactly those three numbers, so
#               writing chat's halved 25 into that file would both collide with
#               the agent tier and send every profile lookup to the fallback.
#               Reusing the file is deliberate -- the request stream is then
#               byte-identical to the t75 arms it is read against, and a second
#               file name for one request stream is its own failure mode.
#   PolyServe   takes mix_short_m1_c25d50s38fair.json, which STATES 25 / 50 / 38,
#               because it reads slo.<class>.tbt_ms literally as the per-token
#               target and set_scheduler_profiling.tier_by_class() derives
#               --polyserve-tier-decode-tokens from the same field. One key in
#               ARMMIX moves both, through PS_MIX_CONFIG in run_cell.
#   scoring     FS_CHAT_TBT_MS=25 FS_DR_TBT_MS=50 FS_SWE_TBT_MS=38 FS_SWE_TTFT_S=7
#               for every table built from these runs, for BOTH arms.
#               exp22_fluidserve.py prints a banner for each on import and refuses
#               a fractional value. Numbers scored this way must never sit in a
#               table with numbers scored at 50 / 100 / 75.
#
# (Original header follows.)
...
declare -A MIXCFG=(
  ...
  # EXP-121. The halved per-token form: chat 25, deepresearch 50, swe 38 ms per
  # token, first-token budgets unchanged. Only the *fair* file exists, because
  # only the policies that read slo.<class>.tbt_ms as the budget need to be told
  # in the file. FluidServe is told through --fluidserve-class-budgets and keeps
  # t75 above, whose tbt_ms are the tier keys its length profile is indexed by --
  # so the two arms below run the SAME request stream and differ only in what
  # each policy was told the budgets are.
  [c25d50s38fair]=/work/workload_configs/mix_short_m1_c25d50s38fair.json
  # EXP-123. The halved per-token budgets AND the halved first-token budgets.
  # Both arms need a file here, unlike EXP-121: a first-token budget has no flag
  # anywhere, so it can only reach either policy as slo.<class>.ttft_ms in the
  # file the runner loads, packed by the client into `priority` and unpacked by
  # the gateway. The FluidServe file keeps tbt_ms at the tier keys 50 / 100 / 25
  # and gets its per-token budgets from --fluidserve-class-budgets; the fair file
  # states both budget sets literally for the policies that read the field as the
  # budget. Everything outside the slo block is identical to t75 in both, so the
  # request stream is the same and only the packed priority integer differs.
  [ftc2500d5000s3500]=/work/workload_configs/mix_short_m1_ftc2500d5000s3500.json
  [c25d50s38ftc2500d5000s3500fair]=/work/workload_configs/mix_short_m1_c25d50s38ftc2500d5000s3500fair.json
...
set_arm() {  # $1 = fluidserve | polyserve | polyservep* | slo | loadbalance
  ...
  case "$1" in
    ...
    fsv3capgnofrct75) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false \
                                          ...
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
                                          FS_SWE_TBT_MS=75 ;;
    # EXP-121. fsv3capgnofrct75 with every per-token budget halved and every
    # first-token budget left alone. It is one change away from that arm and the
    # change is entirely in --fluidserve-class-budgets: the workload file, the
    # profile, the fleet and every other flag are identical, so a difference
    # between the two columns is the budget and nothing else. The name states the
    # three budgets rather than a factor, because "half" is only readable next to
    # the arm it halves, and a result table is read years later on its own.
    #   chat 50 -> 25   FS_CHAT_TBT_MS=25 -> 50:decode:25
    #   deepresearch 100 -> 50   FS_DR_TBT_MS=50 -> 100:decode:50
    #   swe 75 -> 38    FS_SWE_TBT_MS=38 -> 25:decode:38 (25 is the tier key)
    fsv3capgnofrcc25d50s38) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false \
                                          FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false \
                                          FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true \
                                          FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true \
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
                                          FS_SWE_TBT_MS=38 FS_CHAT_TBT_MS=25 FS_DR_TBT_MS=50 ;;
    # EXP-123. The same arm again with the FIRST-TOKEN budgets halved too. The
    # environment below is character-identical to fsv3capgnofrcc25d50s38: every
    # flag, including --fluidserve-class-budgets, is the same, because no flag
    # carries a first-token budget. The ONLY difference between the two arms is
    # which workload file ARMMIX pairs each with, and therefore the packed
    # priority integer each request arrives with -- ttft_ms*1000 + tbt_ms, which
    # the gateway unpacks into SchedulingRequest.TtftSloMs.
    #   chat 5000 -> 2500, deepresearch 10000 -> 5000, swe 7000 -> 3500 ms
    # It changes FluidServe's decisions and not only its score: canWait tests
    # `waited + after > req.ttftSloMs` (fluidserve.go:2264) and the retry
    # deadline is `req.ttftSloMs - after - req.recheckMs` (fluidserve.go:2686),
    # so halving the budget halves the window in which a held request may wait
    # for a feasible instance, and requests that would have been held and later
    # placed become explicit refusals instead.
    fsv3capgnofrcc25d50s38ftc2500d5000s3500) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false \
                                          FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false \
                                          FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true \
                                          FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true \
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
    ...
    polyservept75)  policy=polyserve; export PS_ADMISSION_BINDS=true \
                     ...
                     PS_PARTITION=elastic PS_PREFER_LOADED=true \
                     PS_KV_ADMISSION=true ;;
    # EXP-121. The same six paper mechanisms as polyservept75, on the halved
    # per-token form. Nothing in this line changes: the halved budgets reach the
    # policy through the workload file alone, because PolyServe reads
    # slo.<class>.tbt_ms as the per-token target and its tier table is derived
    # from the same field. ARMMIX pairs this arm with c25d50s38fair and run_cell
    # exports PS_MIX_CONFIG from that one key, so the requests and the tier table
    # cannot come from different files.
    #   the tier keys MOVE for this arm, unlike FluidServe's: chat becomes tier
    #   25, swe tier 38, deepresearch tier 50, and the derived
    #   --polyserve-tier-decode-tokens becomes 25:403,38:464,50:1094 (chat, swe,
    #   deepresearch mean output tokens from fluidserve.json). That is correct
    #   here because PolyServe never reads fluidserve.json's tpot_slo_ms; the
    #   tier is only a bin label whose budget is the number itself.
    polyservepc25d50s38)  policy=polyserve; export PS_ADMISSION_BINDS=true \
                     PS_STEADY_NO_PREFILL=true PS_PROMOTION=true \
                     PS_PARTITION=elastic PS_PREFER_LOADED=true \
                     PS_KV_ADMISSION=true ;;
    # EXP-123. The same six paper mechanisms again, with the first-token budgets
    # halved as well. Nothing in this line changes and nothing can: both budget
    # sets reach PolyServe through the workload file alone. Its admission filter
    # reads the per-request first-token budget through
    # effectiveSloMs(req.ttftSloMs, --ttft-slo) (polyserve.go:525, :696), which
    # prefers the request's own value whenever it is positive, so the global
    # --ttft-slo 5000 that set_scheduler_profiling always sets is the fallback
    # for requests carrying no packed SLO and every request here carries one.
    # The derived --polyserve-tier-decode-tokens is unchanged from EXP-121 at
    # 25:403,38:464,50:1094, because tier_by_class() derives it from tbt_ms and
    # only ttft_ms moved.
    polyservepc25d50s38ftc2500d5000s3500)  policy=polyserve; export PS_ADMISSION_BINDS=true \
                     PS_STEADY_NO_PREFILL=true PS_PROMOTION=true \
                     PS_PARTITION=elastic PS_PREFER_LOADED=true \
                     PS_KV_ADMISSION=true ;;
...
run_cell() {  # $1 arm, $2 mix key, $3 rates, $4 durmin
  ...
  # "bench-runner-" and not "bench-runner-exp106-": the Job controller names each
  # pod "<job name>-<5 characters>" and a pod name is one DNS label capped at 63
  # characters, so with a 39-character arm name the longer prefix produces a
  # 65-character pod name and the Job creates no pods while reporting nothing
  # useful. The substring "bench-runner" is retained because every chain's
  # "cluster is idle" gate counts incomplete Jobs whose name contains it.
  local job="bench-runner-${arm}"
...
declare -A ARMMIX=(
  ...
  [fsv3ftml]=t75
  [fsv3ftms85]=t75
  # EXP-121. The halved per-token budgets. The two arms take DIFFERENT files on
  # purpose and the difference is not a budget difference: FluidServe's file
  # carries tier keys and its budgets arrive through the flag, PolyServe's file
  # carries the budgets themselves. Both arms are judged at chat 25 /
  # deepresearch 50 / swe 38 ms per token.
  [fsv3capgnofrcc25d50s38]=t75
  [polyservepc25d50s38]=c25d50s38fair
  # EXP-123. Both first-token budgets halved as well. BOTH arms take a new file
  # here, which is the difference from the pair above: EXP-121's FluidServe arm
  # could keep t75 because its per-token budgets arrive through a flag, and a
  # first-token budget has no flag, so it can only arrive in the file.
  [fsv3capgnofrcc25d50s38ftc2500d5000s3500]=ftc2500d5000s3500
  [polyservepc25d50s38ftc2500d5000s3500]=c25d50s38ftc2500d5000s3500fair
)
