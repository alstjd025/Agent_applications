...
DYN_TRACE=/work/traces/dynamic/canonical/dyn60_shift_m2Am1B_b1045_x620.csv
...
# EXP-126: the halved budgets. llm-d reads slo.<class>.tbt_ms literally as the
# per-token target, so this is the same file PolyServe and Llumnix SLO read;
# it must name DYN_TRACE above as its class plan.
DYN_WCFG=/work/workload_configs/mix_dyn60_shift_m2Am1B_b1045_x620_c25d50s38ftc2500d5000s3500fair.json
...
case "${1:-}" in
  ...
  full)
    ...
    sed -e "s/__JOBNAME__/bench-runner-exp113hour/" -e "s/__SESSION__/$session/" \
        -e "s/__ARM__/llmdslotc25d50s38ftc2500d5000s3500/" \
