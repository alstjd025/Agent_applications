...
declare -A MIXCFG=(
  ...
  # EXP-139. SLO scale sweep. The k<NNN> files scale ttft_ms only (tbt_ms stays
  # the TIER KEY 25/50/100, because fluidserve_profile.go indexes classes by
  # tpot_slo_ms and a moved key sends every length lookup to the fallback);
  # the k<NNN>fair files scale ttft_ms AND tbt_ms, which is what the other
  # three policies read as the budget itself. Generated by one function that
  # also emits the --fluidserve-class-budgets string, so the two cannot drift.
  [k070]=/work/workload_configs/mix_short_m1_k070.json
  [k070fair]=/work/workload_configs/mix_short_m1_k070fair.json
  [k090]=/work/workload_configs/mix_short_m1_k090.json
  [k090fair]=/work/workload_configs/mix_short_m1_k090fair.json
  [k100]=/work/workload_configs/mix_short_m1_k100.json
  [k100fair]=/work/workload_configs/mix_short_m1_k100fair.json
  [k110]=/work/workload_configs/mix_short_m1_k110.json
  [k110fair]=/work/workload_configs/mix_short_m1_k110fair.json
  [k130]=/work/workload_configs/mix_short_m1_k130.json
  [k130fair]=/work/workload_configs/mix_short_m1_k130fair.json
  # EXP-139 addendum: the symmetric pair around the deployed budget.
  [k080]=/work/workload_configs/mix_short_m1_k080.json
  [k080fair]=/work/workload_configs/mix_short_m1_k080fair.json
  [k120]=/work/workload_configs/mix_short_m1_k120.json
  [k120fair]=/work/workload_configs/mix_short_m1_k120fair.json
...
set_arm() {  # $1 = fluidserve | polyserve | polyservep* | slo | loadbalance
  ...
  case "$1" in
    ...
    # EXP-139. Identical to fsv3capgnofrct75 except the three per-token budgets
    # are NOT pinned here -- kcell exports them from its own grid table. Abort
    # rather than fall through: an unset budget silently restores the compiled
    # default and the condition becomes a duplicate of k=1.0 under a k<>1 name.
    fsv3capgnofrct75k) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false \
                       ...
                       # set_arm unsets every FS_* budget a few lines above, so
                       # these cannot be inherited from the caller's environment:
                       # kcell passes them as K_CHAT/K_DR/K_SWE and they are
                       # rebuilt here, AFTER the unset. An unset K_* aborts the
                       # condition rather than letting the compiled default make
                       # it a silent duplicate of k=1.0.
                       export FS_CHAT_TBT_MS="${K_CHAT:?K_CHAT unset -- kcell must set it}" \
                              FS_DR_TBT_MS="${K_DR:?K_DR unset -- kcell must set it}" \
                              FS_SWE_TBT_MS="${K_SWE:?K_SWE unset -- kcell must set it}" ;;
  ...
  # EXP-139 adds ONE retry. During pre-flight a policy switch made immediately
  # after another one failed verification once in four attempts and passed on
  # every repeat, which is the shape of the log-read race in CLAUDE.md group F:
  # `kubectl logs deploy/scheduler` can pick the pod that is still terminating,
  # so the start-up line read back belongs to the previous configuration. A
  # retry costs one rollout (about a minute) and only on failure; it does NOT
  # weaken the check, because settings that are actually wrong fail the retry
  # too. The retry is announced so the chain log can be counted afterwards: a
  # retry on most conditions would mean something other than a race.
  if ! python3 "$REPO/ms_dev/scripts/set_scheduler_profiling.py" --policy "$policy" \
       | sed 's/^/[exp108]   /'; then
    echo "[exp108] RETRY: settings did not verify for arm $1; retrying once in 20s"
    sleep 20
    if ! python3 "$REPO/ms_dev/scripts/set_scheduler_profiling.py" --policy "$policy" \
         | sed 's/^/[exp108]   /'; then
...
wait_job() {  # $1 job name, $2 deadline in minutes. 0 done, 1 failed, 2 vanished
  local job=$1 deadline=$(( $2 * 60 / 10 )) i st seen=0 gone=0
  for i in $(seq 1 "$deadline"); do
    # A MISSING JOB IS NOT "NOT FINISHED YET" once it has been seen. The loop
    # below used to read `kubectl get job` into a case on the condition type, so
    # a job that no longer existed produced an empty string, matched neither
    # branch, and was waited on for the whole deadline. EXP-139 k070 rep2 lost
    # 4.8 hours to exactly that: the runner finished at 03:22 and wrote a
    # complete 33,720-row metrics.csv, the Job object disappeared afterwards for
    # a reason that was never established, and this loop sat until 08:10 on a
    # condition that had succeeded. The comment above run_cell's call already
    # recorded the same five-hour stall from EXP-48 in its other form (a job in
    # state Failed), which is why the case has a *Failed* branch at all.
    if ! kubectl -n llumnix get job "$job" >/dev/null 2>&1; then
      if [ "$seen" = 1 ]; then
        gone=$(( gone + 1 ))
        # Three consecutive misses, so a transient API-server error does not
        # end a healthy wait.
        if [ "$gone" -ge 3 ]; then
          echo "[wait_job] $job existed and is now gone; the caller decides from the run directory"
          return 2
        fi
      fi
      sleep 10; continue
...
run_cell() {  # $1 arm, $2 mix key, $3 rates, $4 durmin
  ...
  # in state Failed.
  local wrc=0
  wait_job "$job" 300 || wrc=$?
  if [ "$wrc" = 2 ]; then
    # The Job is gone. Judge the condition by what the runner actually left:
    # a merged metrics.csv with rows in it and no shards/ directory means the
    # run finished and merged (a run killed before the merge leaves the shards
    # behind and a header-only metrics.csv -- CLAUDE.md group D).
    local rd rows
    rd=$(ls -dt "$HOSTWORK"/results/*"${session}"* 2>/dev/null | grep -v PRERUN | head -1)
    rows=0
    [ -n "$rd" ] && [ -f "$rd/metrics.csv" ] && rows=$(( $(wc -l < "$rd/metrics.csv") - 1 ))
    if [ "$rows" -gt 0 ] && [ ! -d "$rd/shards" ]; then
      echo "[exp108] $arm/$mix: job vanished, but $(basename "$rd") holds $rows rows with no unmerged shards -- accepted"
      wrc=0
    else
      echo "[exp108] $arm/$mix: job vanished and there is no complete run directory (rows=$rows)"
      wrc=1
    fi
  fi
  [ "$wrc" = 0 ] \
    || { echo "[exp108] $arm/$mix did not complete"
         kubectl -n llumnix logs "job/$job" --tail=60 2>/dev/null; return 1; }
...
case "${1:-}" in
  ...
  arm)
    ...
    ;;
  # EXP-139. One (arm, SLO scale, repeat) cell at one arrival rate.
  #
  #   kcell <arm> <ktag> <rep> [rate-rpm] [minutes]
  #
  # THE GRID LIVES HERE AND NOWHERE ELSE on the cluster side. The three tables
  # below are the per-token budgets in whole milliseconds, half-up rounded from
  # 50 / 100 / 75 times k; the workload files carry the matching ttft_ms and, in
  # the `fair` variant, the same per-token numbers. Both were written by one
  # generator so the policy and the requests cannot disagree. The SCORER must be
  # given the same six values -- they are printed below for every cell so the
  # log itself carries what the table has to be scored with.
  #
  # Tier keys stay 25 / 50 / 100 in --fluidserve-class-budgets. They name the
  # class in fluidserve.json's classes[].tpot_slo_ms, and a moved key sends every
  # length lookup to the fallback (the widest class) in silence.
  # EXP-139 pre-flight. Applies one cell's SETTINGS and stops, so every budget
  # string and every workload file can be fed to the scheduler once before the
  # chain is armed. EXP-63 lost four and a half hours to a generated profile the
  # scheduler could not parse; this is the same guard on this experiment's
  # generated configuration.
  #
  # MUST be run with the cluster idle: set_arm rewrites the deployment spec and
  # restarts the scheduler, which would overwrite the settings of a condition
  # that is currently measuring (CLAUDE.md group C, EXP-131).
  ksettings)
    check_stack
    a=${2:?usage: ksettings <arm> <ktag>}
    kt=${3:?ktag}
    declare -A KCHAT=( [070]=35 [080]=40 [090]=45 [100]=50  [110]=55  [120]=60  [130]=65 )
    declare -A KDR=(   [070]=70 [080]=80 [090]=90 [100]=100 [110]=110 [120]=120 [130]=130 )
    declare -A KSWE=(  [070]=53 [080]=60 [090]=68 [100]=75  [110]=83  [120]=90  [130]=98 )
    [ -n "${KCHAT[$kt]:-}" ] || { echo "[exp139] ABORT: unknown ktag '$kt'"; exit 1; }
    case "$a" in
      fsv3capgnofrct75k) mix="k${kt}"; export K_CHAT="${KCHAT[$kt]}" K_DR="${KDR[$kt]}" K_SWE="${KSWE[$kt]}" ;;
      *) mix="k${kt}fair" ;;
    esac
    wcfg=${MIXCFG[$mix]:-}
    [ -n "$wcfg" ] || { echo "[exp139] ABORT: no mix '$mix'"; exit 1; }
    [ -s "${HOSTWORK}${wcfg#/work}" ] || { echo "[exp139] ABORT: missing $wcfg"; exit 1; }
    export PS_MIX_CONFIG="${wcfg##*/}"
    set_arm "$a" || { echo "[exp139] ABORT: settings for $a k$kt did not verify"; exit 1; }
    echo "[exp139] ksettings OK  arm=$a ktag=$kt mix=$mix"
    ;;
  kcell)
    check_stack
    a=${2:?usage: kcell <arm> <ktag 070|090|100|110|130> <rep> [rate-rpm] [min]}
    kt=${3:?ktag}; rep=${4:?rep}; rate=${5:-2100}; dur=${6:-8}
    case "$rate" in *[!0-9,]*) echo "[exp139] ABORT: rate must be comma-separated rpm: '$rate'"; exit 1 ;; esac
    declare -A KCHAT=( [070]=35 [080]=40 [090]=45 [100]=50  [110]=55  [120]=60  [130]=65 )
    declare -A KDR=(   [070]=70 [080]=80 [090]=90 [100]=100 [110]=110 [120]=120 [130]=130 )
    declare -A KSWE=(  [070]=53 [080]=60 [090]=68 [100]=75  [110]=83  [120]=90  [130]=98 )
    declare -A KTTFT=( [070]="3.5 7 4.9" [080]="4 8 5.6" [090]="4.5 9 6.3" [100]="5 10 7" [110]="5.5 11 7.7" [120]="6 12 8.4" [130]="6.5 13 9.1" )
    # The label is spelled out rather than derived: "0.${kt#0}" reads 0.00 for
    # ktag 100 and 0.10 for 110, and the chain log is what gets read back.
    declare -A KLBL=(  [070]=0.7 [080]=0.8 [090]=0.9 [100]=1.0 [110]=1.1 [120]=1.2 [130]=1.3 )
    [ -n "${KCHAT[$kt]:-}" ] || { echo "[exp139] ABORT: unknown ktag '$kt'"; exit 1; }
    case "$a" in
      fsv3capgnofrct75k) mix="k${kt}"
        # Only this arm consumes the budgets as a policy input; the other three
        # read them out of their workload file. Exporting them for a non-
        # FluidServe policy would change that policy's start-up line for no
        # reason and make its conditions incomparable with EXP-108's.
        export K_CHAT="${KCHAT[$kt]}" K_DR="${KDR[$kt]}" K_SWE="${KSWE[$kt]}" ;;
      *) mix="k${kt}fair" ;;
    esac
    [ -n "${MIXCFG[$mix]:-}" ] || { echo "[exp139] ABORT: no mix '$mix'"; exit 1; }
    set -- $(echo "${KTTFT[$kt]}")
    echo "[exp139] cell arm=$a k=${KLBL[$kt]} mix=$mix rep=$rep rate=${rate}rpm"
    echo "[exp139]   SCORE THIS CELL WITH: FS_CHAT_TTFT_S=$1 FS_DR_TTFT_S=$2 FS_SWE_TTFT_S=$3 FS_CHAT_TBT_MS=${KCHAT[$kt]} FS_DR_TBT_MS=${KDR[$kt]} FS_SWE_TBT_MS=${KSWE[$kt]}"
    SESSION_PREFIX="exp139k${kt}r${rep}" \
      run_cell "$a" "$mix" "$rate" "$dur" \
        || { echo "[exp139] k${kt} rep${rep} $a FAILED"; exit 1; }
    echo "[exp139] k${kt} rep${rep} $a DONE $(date -u +%H:%M)"
    ;;
  *)
