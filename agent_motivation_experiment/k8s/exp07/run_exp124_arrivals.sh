#!/bin/bash
# EXP-124 SNAPSHOT of run_exp120_membrake.sh, taken 2026-09-10. Adds four arms
# for the arrivals term in the KV projection; nothing else changes.
#
# WHAT THE ARMS ARE. --fluidserve-arrival-horizons N adds the fourth term to the
# projection: proj = kvLogical + inflow + arrivals - outflow, where arrivals is
# the mean prompt mass this instance was given per horizon over the last N
# horizons. N is a window and not a gain -- the term is the MEAN per horizon, so
# N changes how much noise is averaged away and not how much is charged -- and
# N=0, the compiled default, reproduces the deployed behaviour exactly.
#
#   fsv3ah3      N=3, the shipped 0.95 memory ceiling. The term alone.
#   fsv3ah1      N=1, the shipped 0.95 memory ceiling. The other end of the window
#                sweep: the measured per-instance correlation between one horizon's
#                arrival mass and the next is +0.76 at N=1 and +0.83 at N=3.
#   fsv3ms97     N=0, --fluidserve-memory-safety=0.97. The ceiling move ALONE, so
#                that a joint improvement in the arm below can be attributed. The
#                value comes from the engine and not from the current operating
#                point: over 59,588 scrape intervals on seven runs there are three
#                preemptions below 0.973 of the physical pool, 0.8% of intervals
#                preempt in 0.973-0.980, 7.3% in 0.980-0.990 and 81.6% above 0.995,
#                so 0.973 is where the engine starts preempting and 0.97 sits just
#                below it. The shipped 0.95 is 2.3 points of pool lower than that.
#   fsv3ah3ms97  N=3 with the 0.97 ceiling: the paired configuration.
#
# WHY THE CEILING MOVES AT ALL. Adding a positive term to the left-hand side of
# proj + cost <= capMem without moving capMem is a tightening, and three
# tightenings measured on this fleet on 2026-09-10 all collapsed (EXP-120's level
# test took offered attainment from 70.2 to 28.7 at 210 req/s, a uniform cut to
# 0.85 of the pool took it to 34.9, and EXP-118's first-token correction took
# admitted attainment from 90.0 to 73.3). The 0.95 factor was doing two jobs at
# once: keeping the engine away from preemption, and absorbing the optimism of a
# projection that had no arrivals term. Removing the optimism leaves only the
# first job, and the engine says that job is done at 0.973.
...
set_arm() {  # $1 = fluidserve | polyserve | polyservep* | slo | loadbalance
  ...
  unset FS_PEND FS_SHED FS_AFFINITY FS_AFFINITY_WEIGHT FS_AFFINITY_METRIC FS_CLASS_PIN \
        ...
        FS_MEM_LEVEL FS_MEM_SAFETY FS_PREFILL_FULLITER FS_PREFILL_RESIDENTQ \
        FS_ARRIVAL_HORIZONS
  # The five names on the two lines above were added by EXP-118, EXP-120 and
  # EXP-124 and were NOT in this list when those arms were written. The chain
  # runs one arm per invocation of this file, so nothing carried over in
  # practice; clearing them here removes the dependence on that fact, and it
  # changes no existing arm, because every arm that wants one of them sets it
  # again in the case below and every arm that does not want one never set it.
  ...
  case "$1" in
    ...
    fsv3capgnofrct75) policy=fluidserve; export FS_PREFIX=true FS_FORCE_MARGIN=false \
                                          ...
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
                                          FS_SWE_TBT_MS=75 ;;
    # EXP-124. The arrivals term. Every other flag is pinned identically to
    # fsv3capgnofrct75 above, so each of these differs from the control in one
    # or two named settings and in nothing else.
    fsv3ah3) policy=fluidserve; export FS_ARRIVAL_HORIZONS=3 FS_PREFIX=true FS_FORCE_MARGIN=false \
                                          FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false \
                                          FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true \
                                          FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true \
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
    ...
    fsv3ah1) policy=fluidserve; export FS_ARRIVAL_HORIZONS=1 FS_PREFIX=true FS_FORCE_MARGIN=false \
                                          ...
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
                                          FS_SWE_TBT_MS=75 ;;
    fsv3ms97) policy=fluidserve; export FS_MEM_SAFETY=0.97 FS_PREFIX=true FS_FORCE_MARGIN=false \
                                          FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false \
                                          FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true \
                                          FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true \
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
                                          FS_SWE_TBT_MS=75 ;;
    fsv3ah3ms97) policy=fluidserve; export FS_ARRIVAL_HORIZONS=3 FS_MEM_SAFETY=0.97 FS_PREFIX=true FS_FORCE_MARGIN=false \
                                          FS_CLASS_HARM=false FS_OWN_BUDGET_GATE=false \
                                          FS_AFFINITY_METRIC=count FS_PER_INSTANCE_CORR=true \
                                          FS_MEMORY_PACE_CAP=true FS_PREFILL_INTERLEAVE=true \
                                          FS_INSTANCE_CAP=true FS_CAP_WINDOW_MULT=3.0 FS_FORCE=false \
...
declare -A ARMMIX=(
  ...
  # lengths at the shipped memory ceiling plus the ceiling move alone and the
  # pairing. Same workload file as the control they are read against
  # (fsv3capgnofrct75), so the request stream is identical.
  [fsv3ah3]=t75
  [fsv3ah1]=t75
  [fsv3ms97]=t75
  [fsv3ah3ms97]=t75
