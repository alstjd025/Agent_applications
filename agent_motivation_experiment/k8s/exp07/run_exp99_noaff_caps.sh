...
set_arm() {
  ...
  unset FS_PEND FS_SHED FS_AFFINITY FS_AFFINITY_WEIGHT FS_AFFINITY_METRIC FS_CLASS_PIN \
        ...
        FS_PER_INSTANCE_CORR FS_MEMORY_PACE_CAP
  case "$1" in
    ...
    # EXP-99. The empty cell of the 2x2. EXP-98's three arms all run with the
    # class preference ON, so they can say what the two changes are worth ON TOP
    # of it and cannot say whether the changes are worth anything WITHOUT it --
    # that is, whether the +7.3 points are the preference's own damage being
    # undone or something the two changes do on their own.
    #
    # Both changes are independent of the preference in the code: enableAffinity
    # guards only the sort's share/count term, while perInstanceCorrection lives
    # in the capacity model and memoryUsesPaceCap in the memory predicate. So
    # this arm is well defined.
    #
    # Every export matches fsnoaff exactly, plus the two flags. FS_AFFINITY_METRIC
    # is deliberately NOT set: with the preference off the metric selects nothing,
    # and EXP-93's fsnoaff did not set it either, so setting it here would be a
    # second difference from the control.
    fsnaboth)    policy=fluidserve; export FS_PREFIX=true FS_AFFINITY=false FS_CLASS_HARM=false FS_FORCE_MARGIN=false FS_OWN_BUDGET_GATE=false \
                                          FS_PER_INSTANCE_CORR=true FS_MEMORY_PACE_CAP=true ;;
