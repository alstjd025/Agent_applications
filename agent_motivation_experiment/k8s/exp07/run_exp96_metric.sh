#!/bin/bash
# EXP-96 SNAPSHOT of run_exp94_wsweep.sh (itself a snapshot of run_exp92_m5.sh), taken 2026-08-24, so the original stays
...
set_arm() {  # $1 = fluidserve | polyserve | slo | loadbalance
  ...
  case "$1" in
    ...
    # EXP-96. The class preference with its normalisation replaced.
    #
    # classShare divides by the instance's OWN occupancy, so it is a ratio and
    # saturates at 1.0: every instance the class already dominates scores the
    # same, the comparator falls through to the free-space tie-break, and that
    # prefers the EMPTIER instance. The class is therefore spread over the
    # instances it has taken rather than filling one, which is the opposite of
    # what the note above sortCandidates describes. Reconstructed from residency
    # on the mix-shift trace, the top share was exactly tied on 45.7% of chat
    # placements in the 93%-chat segment against 1.2% when the same instants are
    # ranked by count.
    #
    # `count` divides by the largest count among the candidates instead. It does
    # not saturate, stays in 0..1 so the weighted sum with free space is still
    # between commensurable quantities, and at weight 1 orders by how many of the
    # class an instance holds.
    #
    # ⚠ This flag exists only in the binary deployed for EXP-96 (md5 8faedded).
    # Do not pin it in an arm that an older binary will run: pflag exits on an
    # unknown flag and the scheduler CrashLoopBackOffs, which the driver reports
    # as "reported no policy" and cannot be told from a slow rollout.
    fscount)     policy=fluidserve; export FS_AFFINITY_METRIC=count FS_CLASS_HARM=false FS_FORCE_MARGIN=false FS_OWN_BUDGET_GATE=false FS_PREFIX=true ;;
