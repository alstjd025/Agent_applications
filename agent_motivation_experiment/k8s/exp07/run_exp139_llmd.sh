...
# EXP-139. Overridable so the SLO-scale chain can hand this arm the k<NNN>fair
# file for the cell it is running. llm-d receives the budget the same way the
# other policies do -- the client packs priority = ttft_ms*1000 + tbt_ms out of
# this file's slo block -- so the file IS how the scale reaches this arm, and a
# forgotten override would run k=1.0 under a k<>1 session name.
WCFG=${WCFG:-/work/workload_configs/mix_short_m1_t75fair.json}
...
# --- job control -------------------------------------------------------------
wait_job() {  # $1 job, $2 deadline minutes. 0 done, 1 failed, 2 vanished
  local job=$1 deadline=$(( $2 * 60 / 10 )) i st seen=0 gone=0
  for i in $(seq 1 "$deadline"); do
    # Same fix as the FluidServe driver: a job that has been seen and is now
    # missing is terminal, not "still running". Reading a missing job as
    # unfinished cost EXP-139 k070 rep2 4.8 hours on a condition that had
    # already written a complete metrics.csv.
    if ! kubectl -n $NS get job "$job" >/dev/null 2>&1; then
      if [ "$seen" = 1 ]; then
        gone=$(( gone + 1 ))
        [ "$gone" -ge 3 ] && { say "$job existed and is now gone"; return 2; }
      fi
      sleep 10; continue
    fi
    seen=1; gone=0
...
condition() {  # $1 rate rpm, $2 session
  ...
  # EXP-139. Re-derive the endpoints AFTER the restart, because the engine pod's
  # IP changes with it, and -- the part that matters here -- sync the
  # InferencePool's targetPorts to the live fleet. The EPP routes on the pool,
  # not on the endpoints ConfigMap, and the pool is a separate object that no
  # driver was updating: it was found holding eight ports (8000-8007) while this
  # fleet has four, left over from the eight-instance experiments. Routing a
  # quarter of the traffic at ports nothing listens on would have shown up as
  # llm-d performing badly, which is EXP-114's failure in the other direction.
  python3 /home/nxclab/llumnix_reproduce/ms_dev/scripts/llmd_endpoints.py \
    || { say "ABORT: llm-d endpoints/InferencePool did not verify"; return 1; }
  ...
  local wrc=0
  wait_job "bench-runner-exp108" 60 || wrc=$?
  if [ "$wrc" = 2 ]; then
    local rd rows
    ...
    if [ "$rows" -gt 0 ] && [ ! -d "$rd/shards" ]; then
      say "job vanished, but $(basename "$rd") holds $rows rows with no unmerged shards -- accepted"
      wrc=0
    else
      say "job vanished and there is no complete run directory (rows=$rows)"; wrc=1
    fi
  fi
  [ "$wrc" = 0 ] || {
    say "measurement failed"; kubectl -n $NS logs job/bench-runner-exp108 --tail=60 2>/dev/null; return 1; }
...
case "${1:-}" in
  ...
  # EXP-139. Pre-flight only: every check `one` would run, and then stop. Lets
  # the SLO-scale chain confirm the llm-d stack before twelve hours are armed,
  # without spending a 3 min pre-run and an 8 min measurement to find out.
  preflight)
    preflight || exit 1
    echo "[exp139] llm-d preflight OK   WCFG=$WCFG"
    [ -s "${WCFG/#\/work/$HOSTWORK}" ] || { echo "[exp139] ABORT: $WCFG not on the host"; exit 1; }
    echo "[exp139] workload file present on host"
    ;;
  one)
    preflight || exit 1
