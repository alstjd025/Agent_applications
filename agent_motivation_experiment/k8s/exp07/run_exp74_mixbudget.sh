...
declare -A MIXCFG=(
  ...
  # EXP-75. Same three workloads and the same SLO block as m1; only the
  # request-count ratio differs, so a run on these against a run on m1 differs in
  # composition and nothing else. They exist because the hour-trace advantage
  # decomposes as +26.1 points from chat against -0.7 and -1.1 from the other two,
  # i.e. all of it is the chat term, and chat is 78.1% of requests under m1 -- a
  # level this project chose and has never varied on the post-2026-08-08 workload.
  # m4 puts chat at 57.1% of requests and m5 at 33.3%. The *f variants carry the
  # slofair swe budget for the policies that cannot express an end-to-end budget.
  [m4]=/work/workload_configs/mix_short_m4_chat57.json
  [m4f]=/work/workload_configs/mix_short_m4_chat57_slofair.json
  [m5]=/work/workload_configs/mix_short_m5_equal.json
  [m5f]=/work/workload_configs/mix_short_m5_equal_slofair.json
...
set_arm() {  # $1 = fluidserve | polyserve | slo | loadbalance
  ...
  case "$1" in
    ...
    # ---------------------------------------------------------------- EXP-73
    # The attribution ladder. Every arm below is `fspfx` -- the v0.2 default --
    # with ONE flag changed, so a difference between an arm and `fspfx` is that
    # flag's contribution and nothing else. This file is a snapshot copy of
    # run_exp67_prefix.sh taken on 2026-08-09 because EXP-72 was running and the
    # original could not be edited (CLAUDE.md trap C: bash re-reads a running
    # script from a remembered offset, and the driver's arm table is re-read once
    # per condition even when the chain itself was snapshotted).
    #
    # Why this ladder and not another. motivation_v2.md section 3.3 has one cell
    # of its grid empty -- routing decided per request (B3) with admission that
    # is NOT the same predicate (A2) -- and that cell is exactly
    # `--fluidserve-enable-pend=false`. The design document said before any of
    # this was built that if holding a request at the gateway is worth no more
    # than the repeat spread, the claim that admission and placement have to be
    # one decision does not stand. It is zero lines of code and has never been
    # run: EXP-25's attempt at it was voided by the Go bool-flag trap, where
    # `--flag false` sets the flag TRUE and passes "false" as a positional
    # argument, so the arm that was supposed to have holding disabled shed
    # 15,723 requests.
    #
    # The 2x2 matters more than either arm alone. If the loss from turning both
    # off exceeds the sum of the two single losses, the two mechanisms help each
    # other and the co-design claim has evidence. If it is less, they partly do
    # the same job and the claim has to be weakened. That test needs all four
    # corners, which is why the control is re-run here rather than borrowed from
    # EXP-68: borrowing it would put a different session on one side of a
    # subtraction whose expected size (2-8 points) is the size of the session
    # effect.
    fspnopend)   policy=fluidserve; export FS_CLASS_HARM=false FS_FORCE_MARGIN=false FS_OWN_BUDGET_GATE=false FS_PREFIX=true FS_PEND=false ;;
    fspnoaff)    policy=fluidserve; export FS_CLASS_HARM=false FS_FORCE_MARGIN=false FS_OWN_BUDGET_GATE=false FS_PREFIX=true FS_AFFINITY=false ;;
    # Both off. This is the configuration that corresponds to SLOs-Serve's
    # routing: evaluate each instance against the tightest budget resident on
    # it, place immediately, and do not prefer an instance because it already
    # holds the same class. It is not a port of SLOs-Serve -- their engine-side
    # token allocation is not implemented -- so it must never be labelled as one.
    fspslos)     policy=fluidserve; export FS_CLASS_HARM=false FS_FORCE_MARGIN=false FS_OWN_BUDGET_GATE=false FS_PREFIX=true FS_PEND=false FS_AFFINITY=false ;;
