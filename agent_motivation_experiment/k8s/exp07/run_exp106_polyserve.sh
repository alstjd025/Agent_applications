...
#
#   ./run_exp106_polyserve.sh smoke [rates] [min]     one condition, polyservep,
#                                                     for the section 4.1 checks
#   ./run_exp106_polyserve.sh sweep106 [rates] [min] [reps]
#   ./run_exp106_polyserve.sh arm <arm> <mix> [rates] [min]
#
...
check_stack() {
  ...
  [ "$mig" = "0" ] || { echo "[exp106] ABORT: engine migration is '$mig', want 0"; exit 1; }
  ...
  [ "$extra" = "$want_extra" ] || {
    echo "[exp106] ABORT: engine has SCHED_EXTRA_ARGS='$extra', expected '${want_extra:-<empty>}'"
    exit 1; }
  ...
  [ -s "$SHORT_TRANSCRIPT" ] \
    || { echo "[exp106] ABORT: short transcript missing: $SHORT_TRANSCRIPT"; exit 1; }
  echo "[exp106] stack ok: theta off, gateway=$bin, migration off, engine sched='${want_extra:-stock FIFO}'"
}
...
set_arm() {  # $1 = fluidserve | polyserve | polyservep* | slo | loadbalance
  ...
  case "$1" in
    ...
    *) echo "[exp106] unknown arm: $1" >&2; return 1 ;;
  esac
  echo "[exp106] switching scheduler -> $policy"
  ...
  if ! python3 "$REPO/ms_dev/scripts/set_scheduler_profiling.py" --policy "$policy" \
       | sed 's/^/[exp106]   /'; then
    echo "[exp106] ABORT: set_scheduler_profiling.py reported the applied flags"
  ...
  [ -n "$actual" ] || { echo "[exp106] scheduler pod $pod reported no policy after 60s"; return 1; }
  [ "$actual" = "$policy" ] \
    || { echo "[exp106] ABORT: scheduler reports '$actual', wanted '$policy'"; return 1; }
  echo "[exp106] scheduler confirmed policy=$actual (pod $pod)"
}
...
run_cell() {  # $1 arm, $2 mix key, $3 rates, $4 durmin
  ...
  [ -n "$wcfg" ] || { echo "[exp106] unknown mix '$mix'"; return 1; }
  [ -s "${HOSTWORK}${wcfg#/work}" ] || { echo "[exp106] missing $wcfg"; return 1; }
  ...
  echo "[exp106] $arm/$mix launched $(date -u +%H:%M:%S), rates=$rates rpm dur=${durmin}m"
  ...
  # in state Failed.
  wait_job "$job" 300 \
    || { echo "[exp106] $arm/$mix did not complete"
  ...
  kubectl -n llumnix delete job "$job" --ignore-not-found >/dev/null
  echo "[exp106] $arm/$mix DONE $(date -u +%H:%M:%S)"
}
...
case "${1:-}" in
  smoke)
    ...
    echo "[exp106] SMOKE DONE $(date -u +%H:%M)"
    ;;
  sweep106)
    check_stack
    RATES=${2:-600,900,1200,1500,2100,2700,3300,4200}
    DUR=${3:-8}
    REPS=${4:-2}
    ARMS=${ARMS:-polyservep polyserve}
    # Guard the argument itself: run_experiment.py splits --rate-list on commas
    # and a space-separated list dies on the first float conversion, forty
    # seconds in, after which the chain reports DONE having run nothing.
    case "$RATES" in *[!0-9,]*) echo "[exp106] ABORT: rates must be comma-separated rpm: '$RATES'"; exit 1 ;; esac
    for rep in $(seq 1 "$REPS"); do
      # Repeat is the OUTER loop, so machine drift over the hours is spread
      # across arms instead of being charged to whichever arm ran last.
      for arm in $ARMS; do
        SESSION_PREFIX="${SESSION_PREFIX_BASE:-exp106}r${rep}" \
          run_cell "$arm" "${ARMMIX[$arm]}" "$RATES" "$DUR" \
            || echo "[exp106] rep$rep $arm FAILED"
      done
      echo "[exp106] REPEAT $rep DONE $(date -u +%H:%M)"
    done
    echo "[exp106] SWEEP DONE $(date -u +%H:%M)"
    echo "[exp106] result directories: $(ls -d ../../results/*${SESSION_PREFIX_BASE:-exp106}r* 2>/dev/null | grep -vc PRERUN)"
    ;;
  arm)
    check_stack
