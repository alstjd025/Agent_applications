#!/bin/bash
# EXP-114 SNAPSHOT of run_exp114_knee.sh with the load generator running 24
# worker processes instead of 12.
#
# WHY. The per-token time the client measures is proportional to the number of
# concurrent streams, not to what the engine does: at 130 req/s under FluidServe
# the engine serving 946 requests reports 43.6 ms per token and the one serving a
# single request reports 4.7 ms, and the client measures 59.2 and 67.2 ms for
# them. Across every condition at 100-130 req/s that measured time divided by
# streams-per-client-process is 0.19-0.26 ms, and the same ratio taken at the
# GATEWAY is 0.11-0.16 ms. So a shared multiplexer is serialising per-token work,
# part of it in the gateway -- which is the system under test -- and part in the
# runner, which is not.
#
# Doubling the runner's processes halves the streams each one carries. If the
# client's share is real, its measured per-token time falls toward the gateway's
# and the ratio per stream drops; if nothing moves, the whole effect belongs to
# the gateway and the runner is exonerated.
#
# Everything else is identical to run_exp114_knee.sh, including the arms, so the
# 12-process runs already on disk are the control.
# EXP-114 SNAPSHOT of run_exp114_capablate.sh, taken 2026-09-04 after the llumlet
# client-index fix made --api-server-count 4 usable.
#
# WHY A SNAPSHOT. The file it comes from is on the measurement path of EXP-108,
# EXP-109 and the count=1 conditions of EXP-114, and this run adds an arm to the
# ARMMIX table. Adding a row is additive, but the rule in this repository is that
# a driver whose results are already published is not edited (CLAUDE.md group C).
#
# WHAT THIS MEASURES. Two things the front-end bottleneck made unmeasurable:
#   1. Llumnix load-balance at four API server processes per engine. The number
#      on record (offered 99.8 at 120 req/s) was taken at ONE, where that policy
#      was the least affected of any -- it spreads load over all eight front ends
#      instead of concentrating a class onto two -- so it is the comparison that
#      the bottleneck distorted most, and in our favour to correct.
#   2. Where FluidServe's capacity knee now is. At count=1 it scored 55.5 at
#      100 req/s; at count=4 it scores 99.3 there, so the knee is ABOVE 100 and
#      its position is unknown.
#
# The two arms run on one grid with one discipline -- eight minutes, an engine
# cold restart per condition -- because the count=1 load-balance ladder used
# three minutes in a single job with no restart between rates, and a knee read
# off two different disciplines is not a comparison.
#
# ⚠ THE TOP RATE MAY MEASURE THE CLIENT. The load generator was clean to
# 200 req/s and broke at 300 (16.3% connection errors); 250 is untested. Any
# point at 250 has to be checked for connection errors before it is read as a
# property of the fleet -- exp114_probe_report.py counts them.
...
run_cell() {  # $1 arm, $2 mix key, $3 rates, $4 durmin
  ...
  sed -e "s/__JOBNAME__/$job/" -e "s/__SESSION__/$session/" \
      ...
      -e "s#__WCFG__#$wcfg#" \
      runner-exp114proc.template.yaml | kubectl apply -f - >/dev/null
...
declare -A ARMMIX=(
  ...
  [slot75]=t75fair
  # EXP-114 (2026-09-04). Llumnix load-balance on the eight-instance fleet, to be
  # read against fsv3capgnofrct75 at --api-server-count 4. It gets the t75 file
  # rather than t75fair even though it is not a FluidServe arm: it reads no SLO
  # at all -- neither the scheduler policy nor the stock engine consumes
  # slo.<class>.tbt_ms -- so the field cannot change its decisions, and the two
  # files differ in nothing else. Giving it the same file as the arm it is
  # compared with keeps the request stream identical, and it is also the file the
  # count=1 ladder used, so the two counts stay comparable for this arm too.
  [loadbalance]=t75
)
...
case "${1:-}" in
  ...
  onerep)
    ...
    SESSION_PREFIX="${SESSION_PREFIX_BASE:-exp114cap}r${rep}" \
