...
DYN_TRACE=/work/traces/dynamic/canonical/dyn60_shift_m2Am1B_b1045.csv
...
DYN_WCFG=/work/workload_configs/mix_dyn60_shift_m2Am1B_b1045_t75fair.json
...
case "${1:-}" in
  ...
  full)
    ...
    kubectl -n $NS delete job bench-runner-exp109hour --ignore-not-found >/dev/null
    sed -e "s/__JOBNAME__/bench-runner-exp109hour/" -e "s/__SESSION__/$session/" \
    ...
    wait_job "bench-runner-exp109hour" 120 || {
      say "hour trace failed"; kubectl -n $NS logs job/bench-runner-exp109hour --tail=60; exit 1; }
    kubectl -n $NS logs job/bench-runner-exp109hour --tail=400 2>/dev/null | grep -aE "Success:|rc=" | tail -3
    kubectl -n $NS delete job bench-runner-exp109hour --ignore-not-found >/dev/null
    collect "$session" "$off"
