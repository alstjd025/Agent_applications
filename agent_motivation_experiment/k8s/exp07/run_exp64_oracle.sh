...
set_arm() {  # $1 = fluidserve | polyserve | slo | loadbalance
  ...
  case "$1" in
    ...
    # amount".
    # EXP-64. The per-request length oracle. The ONLY difference from fspfx is
    # --fluidserve-oracle-length: both arms send the hint in the OpenAI user
  ...
  # ⚠ The exit status is CHECKED, and it was not until 2026-08-11.
  #
  # set_scheduler_profiling.py reads the scheduler's start-up line back and
  # returns 1 when the configuration it reports differs from the one applied.
  # That return value was thrown away here: the call is a pipeline, and although
  # `set -o pipefail` makes the pipeline's status 1, nothing tested it. The only
  # check that followed compares the POLICY NAME, which is right in exactly the
  # case the verifier exists for -- an ablation flag that failed to apply leaves
  # the name alone and turns the arm into its control. So the condition ran with
  # the wrong configuration and the failure was visible only to whoever read the
  # log afterwards.
  #
  # The verifier caught a real mismatch on EXP-79's first pre-flight (its own
  # regex could not read a value containing a colon), which is what exposed this.
  if ! python3 "$REPO/ms_dev/scripts/set_scheduler_profiling.py" --policy "$policy" \
       | sed 's/^/[exp27]   /'; then
    echo "[exp27] ABORT: the scheduler is not running the configuration it was given"
    return 1
  fi
  local pod actual i
...
run_cell() {  # $1 arm, $2 mix key, $3 rates, $4 durmin
  ...
  sed -e "s/__JOBNAME__/$job/" -e "s/__SESSION__/$session/" \
      ...
      runner-exp64.template.yaml | kubectl apply -f - >/dev/null
