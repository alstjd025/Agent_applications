...
| (b) | 그 순간의 구성이 **창 평균 구성에서 떨어진 거리** (total variation) | 중앙값 **0.115**, p95 0.340, **최대 0.402** |

### (b)를 구성 분해가 아니라 거리로 그리는 이유 셋

1. **정적 파티션이 틀리는 양이 바로 이 값이다.** 평균 구성에 맞춰 크기를 정한 파티션은 매
   순간 정확히 이만큼 어긋나 있다. **y축이 trace의 성질이 아니라 그 설계가 지는 오차다.**
...
3. **주장이 "믹스가 움직인다"이지 "코드 쪽으로 움직인다"가 아니다.** 이 파일의 앞 판은 반대로
   ...
   **어떤 고정 파티션도 맞지 않는다**를 세우는 자리라 거리가 내용 전부다.
