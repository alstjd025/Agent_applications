...
## `azure_rate_and_mix.pdf` — 도착률과 구성비가 둘 다 움직이고, 서로를 예측하지 못한다

**스크립트**: `fig_azure_rate_and_mix.py`. **크기**: 3.335 × 2.45 in, 단일 열,
`width=\columnwidth`.

**무엇을 주장하는 그림인가.** `azure_trace_shape.pdf`는 도착률만 보여 주는데, motivation의
주장은 **도착률이 변하고 그 구성도 변한다**는 두 부분이다. 이 그림이 같은 나흘 창에 둘을 같이
놓는다. **구성이 변한다는 것이 정적 클래스 파티션을 틀리게 만드는 것**이다 — 마루 도착률에
맞춰 함대를 준비해도 그때의 구성이 골에서의 구성이 아니다.

| 패널 | 무엇 | 실측 (Azure LLM Inference 2024, 나흘 창, 10분 구간) |
|---|---|---|
| (a) | 도착률 ÷ 그 창의 최대값 | 골이 마루의 **17.3%**, 마루/골 **5.8배** |
| (b) | 도착의 구성, 100%로 쌓음 | 코드가 **3.9% ~ 68.0%**(p5 5.9 / 중앙값 23.2 / p95 61.8, **p95/p5 10.54배**), **나머지 전부가 대화형** |

**(b)를 왜 한 줄짜리 비율이 아니라 누적 100%로 그리는가.** "코드 요청 %"라는 선 하나는
**나머지가 무엇인지 말하지 않는데, 나머지가 요점이다** — 이 패널이 보이려는 것은 구성이므로
두 성분이 다 있어야 한다. Azure 2024 공개본은 대화형과 코드 정확히 둘이라 **두 띠가 분포
전체이고, 그 경계선이 움직이는 것이 곧 주장이다.** 띠 이름은 범례가 아니라 띠 안에 직접
적었다 — 범례를 축 위에 두면 위쪽 띠와 겹쳐 그림 위에 쓴 글자로 읽힌다.

**섞인 정도를 나타내는 지표를 대신 그리지 않는 이유.** 클래스가 둘이면 **비율 하나가 분포
전체를 결정하므로**, 얼마나 섞였는지를 나타내는 지표(이 저장소가 인스턴스별 클래스 분포에
이미 쓰는 유효 클래스 수 `1/Σs²`)는 **비율이 이미 담고 있는 것을 더하지 않고 방향을 잃는다** —
코드 20%와 80%를 똑같이 1.47로 읽는다. **한쪽에 맞춰 크기를 정한 파티션은 다른 쪽에서
틀리므로 방향이 정확히 살아남아야 하는 것이다.** 참고로 그 지표는 이 창에서 최소 1.08 /
중앙값 1.55 / 최대 2.00(클래스가 둘이라 상한이 2.00)이고, 창 평균 구성으로부터의 평균 total
variation 거리는 0.135다. **둘 다 스크립트가 출력하고 그리지는 않는다.**
...
2. **두 패널 다 원본 trace이고 우리가 재생하는 trace가 아니다.** 재생 trace의 도착률은
   분위수 변환을 우리가 고른 대역에 씌운 것이라 마루/골 비가 우리가 정한 값이고, 재생
   trace의 구성은 **세 클래스짜리 합성 스케줄**이다. **둘 다 그리면 순환이다.**
3. **Azure에는 클래스가 둘(대화형·코드)뿐이고 deepresearch에 해당하는 것이 없다.**
   그러므로 이 그림은 **구성이 움직인다는 근거이지, 우리가 고른 수준(10:2:1)이나 세 클래스
   분해의 근거가 아니다**(`motivation.md` §7.3.1의 한계와 같다).
4. **p95/p5 10.54배는 이 나흘 창의 값이다.** `traces/azure/mix_over_time.txt`의 8.67배는
   168시간 전체의 값이라 다른 수다. **어느 창의 값인지 밝힌다.**
