#!/usr/bin/env bash
# EXP-105 hour trace: FluidServe v0.3 with the agent class end-to-end budget raised
# from 30 s to 40 s, class preference on against off, two repeats each. A snapshot
# of redraw_hour_trace_exp104.sh rather than an argument to it.
#
# The comparison this set is FOR is the preference within the 40 s budget. The
# 30 s runs are NOT drawn beside these: swe is scored against a different rule in
# each, so its curves are not the same quantity. Use exp105_budget_affinity_grid.py
# for the cross-budget table, which prints the budget it scored each arm at.
...
set -uo pipefail
cd "$(dirname "$0")/.." || exit 1
OUT=${1:-results/aggregate_analysis/exp105_swebudget40}
PASS=${2:-}
R=analysis_scripts/request_level
mkdir -p "$OUT"

SH=$(ls -d results/*exp105r1_fsv3b40_shift 2>/dev/null | head -1)
CO=$(ls -d results/*exp105r2_fsv3b40_shift 2>/dev/null | head -1)
BO=$(ls -d results/*exp105r1_fsv3noaffb40_shift 2>/dev/null | head -1)
NO=$(ls -d results/*exp105r2_fsv3noaffb40_shift 2>/dev/null | head -1)
...
MISSING=0
for pair in "v0.3 swe 40 s, preference on rep 1|$SH" "v0.3 swe 40 s, preference on rep 2|$CO" "v0.3 swe 40 s, preference off rep 1|$BO" "v0.3 swe 40 s, preference off rep 2|$NO"; do
  name=${pair%%|*}; dir=${pair#*|}
...
SERIES=(); HOUR=(); RUNS=()
[ -n "$SH" ] && { SERIES+=("v0.3 swe 40 s, preference on, rep 1|#1f77b4|-|$SH");   HOUR+=("v0.3 swe 40 s, preference on, rep 1|#1f77b4|$SH");   RUNS+=("$SH"); }
[ -n "$CO" ] && { SERIES+=("v0.3 swe 40 s, preference on, rep 2|#17becf|-|$CO");     HOUR+=("v0.3 swe 40 s, preference on, rep 2|#17becf|$CO");     RUNS+=("$CO"); }
[ -n "$NO" ] && { SERIES+=("v0.3 swe 40 s, preference OFF, rep 2|#d62728|--|$NO");   HOUR+=("v0.3 swe 40 s, preference OFF, rep 2|#d62728|$NO");   RUNS+=("$NO"); }
[ -n "$BO" ] && { SERIES+=("v0.3 swe 40 s, preference OFF, rep 1|#ff7f0e|--|$BO");  HOUR+=("v0.3 swe 40 s, preference OFF, rep 1|#ff7f0e|$BO");  RUNS+=("$BO"); }
...
python3 traces/dynamic/plot_trace_shape.py \
  ...
  --title "EXP-105 hour trace: the same Azure-shaped arrival band as EXP-93, 10.8-45.0 req/s, with the class mix stepping m2 A m1 B every 15 minutes so chat runs 93.0, 33.3, 76.9 and 60.0 percent of the requests. The knee is m1's measured 28.0 req/s; the other three segments have their own and none is measured." \
  --out "$OUT/trace_shape.png" >/dev/null 2>&1 && echo "  ok" || echo "  FAILED"

echo
echo "=== fleet timeline"
python3 "$R/exp41_dynamic_timeline.py" --variant shift --out-dir "$OUT" \
  --title "EXP-105, v0.3 class preference on against off" --series "${SERIES[@]}" \
  && echo "  ok" || echo "  FAILED"
...
[ -n "$NO" ] && true
[ -n "$SH" ] && EV+=("fsv3b40=$SH")
[ -n "$CO" ] && EV+=("fsv3b40_r2=$CO")
[ -n "$BO" ] && EV+=("fsv3noaffb40=$BO")
[ -n "$NO" ] && EV+=("fsv3noaffb40_r2=$NO")
[ -n "$LD" ] && true
python3 "$R/exp41_engine_view.py" --variant shift --out-dir "$OUT" \
  --run "${EV[@]}" --title "EXP-105" \
  && echo "  ok" || echo "  FAILED"

echo
echo "=== the four runs side by side"
python3 "$R/exp38_policy_compare.py" --runs "${RUNS[@]}" --tag hour \
  --out-dir "$OUT" --title "EXP-105 hour trace, v0.3 class preference on against off" \
